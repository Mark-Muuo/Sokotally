...
const KEYWORD_PATTERNS = {
  sales: {
    keywords: ['sale', 'sales', 'sold', 'revenue', 'income', 'earning', 'bought'],
    category: 'sales'
  },
  expenses: {
    keywords: ['expense', 'expenses', 'cost', 'spent', 'payment', 'paid', 'bill', 'purchase'],
    category: 'expenses'
  },
  debt: {
    keywords: ['debt', 'owe', 'owes', 'credit', 'loan', 'borrow', 'lend', 'due'],
    category: 'debt'
  },
  customer: {
...
export function extractKeywords(text) {
  ...
  if (amounts) {
    extracted.amounts = amounts.map(a => parseFloat(a.replace(/[^\d.]/g, ''))).filter(Boolean);
  }
  ...
  if (names) {
    const exclude = ['I', 'Today', 'Yesterday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'];
...
export function buildContextFromKeywords(keywords) {
  let prompt = "You are a bookkeeping assistant for Sokotally. ";
  
  if (keywords.categories.length > 0) {
    prompt += `User is asking about: ${keywords.categories.join(', ')}. `;
  }
  
  if (keywords.action === 'create' && keywords.amounts.length > 0) {
    prompt += "The user wants to record a transaction. Extract the details (amount, description, customer/item) and confirm what will be recorded. ";
  }
