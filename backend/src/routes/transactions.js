...
router.get('/stats/dashboard', authMiddleware, async (req, res, next) => {
  try {
    ...
    const [todaySalesAgg] = await Transaction.aggregate([
      { $match: { userId, type: 'income', occurredAt: { $gte: todayStart, $lte: todayEnd } } },
      { $group: { _id: null, total: { $sum: '$amount' } } }
    ]);

    const [todayExpensesAgg] = await Transaction.aggregate([
      { $match: { userId, type: 'expense', occurredAt: { $gte: todayStart, $lte: todayEnd } } },
      { $group: { _id: null, total: { $sum: '$amount' } } }
    ]);

    // Yesterday's sales and expenses for comparison
    const [yesterdaySalesAgg] = await Transaction.aggregate([
      { $match: { userId, type: 'income', occurredAt: { $gte: yesterdayStart, $lte: yesterdayEnd } } },
      { $group: { _id: null, total: { $sum: '$amount' } } }
    ]);

    const [yesterdayExpensesAgg] = await Transaction.aggregate([
      { $match: { userId, type: 'expense', occurredAt: { $gte: yesterdayStart, $lte: yesterdayEnd } } },
      { $group: { _id: null, total: { $sum: '$amount' } } }
    ]);
...
router.get('/stats/report', authMiddleware, async (req, res, next) => {
  try {
    ...
    const recentTransactions = await Transaction.find(dateFilter).sort({ occurredAt: -1 }).limit(100).lean();

    res.json({
      sales,
      expenses,
      profit,
      period: period === 'month' && start ? `${start.toLocaleDateString('en-US', { year: 'numeric', month: 'long' })}` : period === 'all' ? 'All Time' : 'Custom Range',
      dateRange: start && end ? { start: start.toISOString(), end: end.toISOString() } : null,
      debts: {
        outstandingTotal: outstandingDebts,
        outstandingCount: outstandingDebtsCount
      },
...
router.get('/reports/ai', authMiddleware, async (req, res, next) => {
  try {
    ...
    const summary = {
      ...
      topItems: topItems.map(t => ({ name: t._id, quantity: t.quantity, revenue: t.revenue, unit: t.unit })),
      daily: dailyBreakdown
    };

    // Compose prompt
    const systemPrompt = 'You are a business analyst creating a clear, concise report for a small shop. Keep it simple and actionable.';
...
Data (JSON):\n\n${JSON.stringify(summary, null, 2)}\n\nRequirements:\n- Sections: Overview, Key Metrics, Top Items, Insights & Recommendations.\n- Currency is KSh. Use thousands separators.\n- Keep language simple and friendly.\n- For PDF format, use plain text with clear section headings and bullet points (no markdown syntax).`;
    ...
    try {
      ...
      lines.push(`- Total Expenses: KSh ${summary.totals.expenses.toLocaleString()}`);
      ...
      if (summary.topItems.length) {
        ...
        lines.push('|---|---:|---:|');
        summary.topItems.forEach(it => lines.push(`| ${it.name} | ${it.quantity} ${it.unit || ''} | KSh ${it.revenue.toLocaleString()} |`));
      } else {
      ...
      if (summary.daily.length) {
        ...
        lines.push('|---|---|---:|');
        summary.daily.forEach(d => lines.push(`| ${d._id.date} | ${d._id.type} | KSh ${d.total.toLocaleString()} |`));
      } else {
    ...
    if (format === 'pdf') {
      ...
      res.setHeader('Content-Disposition', `attachment; filename=sokotally-ai-report-${new Date().toISOString().split('T')[0]}.pdf`);

      // Parse content for PDF rendering
      doc.fontSize(20).font('Helvetica-Bold').text('SokoTally AI Business Report', { align: 'center' });
      doc.moveDown();
      doc.fontSize(10).font('Helvetica').text(`Period: ${new Date(summary.period.start).toLocaleDateString()} - ${new Date(summary.period.end).toLocaleDateString()}`, { align: 'center' });
      doc.moveDown(2);
      ...
      if (summary.topItems.length > 0) {
        summary.topItems.forEach((item, idx) => {
          doc.text(`${idx + 1}. ${item.name} - Qty: ${item.quantity} ${item.unit || ''} - Revenue: KSh ${item.revenue.toLocaleString()}`);
        });
      } else {
      ...
      // AI Insights Section
      doc.fontSize(14).font('Helvetica-Bold').text('AI Insights & Recommendations');
      doc.moveDown(0.5);
      doc.fontSize(10).font('Helvetica');
      ...
      if (aiGenerated) {
        ...
        const insightsMatch = cleanText.match(/insights?[\s&]*recommendations?[:\n]+([\s\S]+)/i);
        if (insightsMatch) {
    ...
    if (format === 'html') {
      ...
      if (download === '1') {
        res.setHeader('Content-Disposition', `attachment; filename="sokotally-ai-report-${new Date().toISOString().split('T')[0]}.html"`);
      }
    ...
    if (download === '1') {
      res.setHeader('Content-Disposition', `attachment; filename="sokotally-ai-report-${new Date().toISOString().split('T')[0]}.md"`);
    }
    return res.send(body);
  } catch (e) {
    next(e);
...
router.get('/reports/pdf', authMiddleware, async (req, res, next) => {
  try {
    ...
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename=sokotally-report-${new Date().toISOString().split('T')[0]}.pdf`);

    doc.fontSize(18).text('SokoTally Business Report', { align: 'center' });
    doc.moveDown();
    doc.fontSize(10).text(`Period: ${start.toLocaleDateString()} - ${end.toLocaleDateString()}`);
    doc.moveDown();
    ...
    if (topItems.length === 0) {
      ...
      topItems.forEach(it => {
        doc.fontSize(10).text(`${it._id} - Qty: ${it.quantity} - Revenue: KSh ${it.revenue.toLocaleString()}`);
      });
    }
...
router.get('/export', authMiddleware, async (req, res, next) => {
  try {
    ...
    const transactions = await Transaction.find(query).sort({ occurredAt: -1 }).limit(5000);
...
router.get('/export/xlsx', authMiddleware, async (req, res, next) => {
  try {
    ...
    transactions.forEach(t => ws.addRow({
