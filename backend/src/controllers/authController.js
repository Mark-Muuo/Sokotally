...
export async function signup(req, res) {
  try {
    const { firstName, lastName, username, email, phone, password, preferredLang } = req.body || {};
    if (!firstName || !lastName || !phone || !password) return res.status(400).json({ error: 'Missing required fields' });
