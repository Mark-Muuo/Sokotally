...
const SignIn = () => {
	...
	return (
		<div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-amber-50/80 via-stone-100 to-amber-50/80 dark:from-[#0b1320] dark:via-[#0e1726] dark:to-[#0b1320] p-6 relative overflow-hidden">
			...
			{/* Form Card */}
			<form onSubmit={handlePasswordLogin} className="bg-white/90 dark:bg-slate-900/80 backdrop-blur-xl p-10 w-full max-w-md shadow-2xl relative z-10 border border-gray-200 dark:border-slate-700/50">
				{/* Logo Header */}
				<div className="text-center mb-8">
				<h1 className="text-4xl font-light text-gray-900 dark:text-white mb-2 tracking-wide">
					SokoTally
				</h1>
				<p className="text-gray-600 dark:text-slate-300 text-sm font-medium">Sign in to your SokoTally account</p>
			</div>

				<h2 className="text-2xl font-light text-gray-900 dark:text-white mb-6 text-center">Welcome back</h2>				{error && (
				...
				<label className="block mb-5">
					...
					<div className="relative flex items-center gap-2">
						<input 
							...
							className={`flex-1 px-4 py-3 border text-base transition-all duration-200 bg-gray-100 dark:bg-slate-800/50 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-slate-500 font-light focus:outline-none focus:bg-gray-50 dark:focus:bg-slate-800 ${
				...
				{otpMode && (
				<div className="mb-5">
					<span className="block mb-2 font-light text-gray-600 dark:text-slate-300 text-sm">Enter the 6-digit code</span>
					<input 
						type="text"
						inputMode="numeric"
						pattern="[0-9]*"
					...
					/>
					<p className="text-gray-500 dark:text-slate-400 text-xs mt-2">We will send a 6-digit code to your phone.</p>
					{validationErrors.otp && (
						<span className="text-red-400 text-sm font-light mt-2 flex items-center gap-2">
							<span>⚠</span>
							{validationErrors.otp}
				...
				<div className="flex justify-between items-center mb-6 text-sm">
					<label className="flex items-center gap-2 font-light text-gray-600 dark:text-slate-300 cursor-pointer hover:text-gray-900 dark:hover:text-white transition-colors">
						<input 
							type="checkbox" 
							checked={rememberMe}
					...
					<Link to="/reset" className="text-blue-600 dark:text-blue-500 font-light hover:text-blue-500 dark:hover:text-blue-400 transition-all">
						Forgot password?
					</Link>
				</div>
				)}
				...
				<div className="text-center mt-6 text-gray-600 dark:text-slate-400 text-sm font-light">
					<p>
						...
						<Link to="/signup" className="text-blue-600 dark:text-blue-500 font-normal hover:text-blue-500 dark:hover:text-blue-400 transition-all">
							Sign up
						</Link>
					</p>
				</div>
